...
#define JNI_CLASS_IJKPLAYER     JNI_MODULE_PACKAGE"/IjkMediaPlayer"
#define JNI_IJK_MEDIA_EXCEPTION JNI_MODULE_PACKAGE"/exceptions/IjkMediaException"
...
static int64_t jni_set_media_data_source(JNIEnv *env, jobject thiz, jobject media_data_source) {
    ...
    jobject
            ...
            J4AC_IjkMediaPlayer__mNativeMediaDataSource__get__catchAll(
                    env, thiz);
    if (old) {
...
static int64_t jni_set_ijkio_androidio(JNIEnv *env, jobject thiz, jobject ijk_io) {
    ...
    jobject
            old = (jobject) (intptr_t)
            J4AC_IjkMediaPlayer__mNativeAndroidIO__get__catchAll(env,
                                                                 thiz);
    if (old) {
    ...
    if (ijk_io) {
        ...
        nativeAndroidIO = (int64_t) (intptr_t)
...
IjkMediaPlayer_native_setup(JNIEnv *env, jobject thiz, jobject weak_this) {
    ...
    void *weak = ijkmp_set_weak_thiz(mp);
    ijkmp_set_inject_opaque(mp, weak);
    ijkmp_set_ijkio_inject_opaque(mp,weak );
    ijkmp_android_set_mediacodec_select_callback(mp, mediacodec_select_callback,weak);
    LABEL_RETURN:
    ijkmp_dec_ref_p(&mp);
}
...
// FindClass returns LocalReference
    IJK_FIND_JAVA_CLASS(env, g_clazz
            .clazz, JNI_CLASS_IJKPLAYER);
    (*env)->
            RegisterNatives(env, g_clazz
                                    .clazz, g_methods,
                            NELEM(g_methods)
    );
